import{a as c,S as R,i as U,s as Y}from"./assets/vendor-BeE4JKq3.js";(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const o of a)if(o.type==="childList")for(const l of o.addedNodes)l.tagName==="LINK"&&l.rel==="modulepreload"&&n(l)}).observe(document,{childList:!0,subtree:!0});function s(a){const o={};return a.integrity&&(o.integrity=a.integrity),a.referrerPolicy&&(o.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?o.credentials="include":a.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function n(a){if(a.ep)return;a.ep=!0;const o=s(a);fetch(a.href,o)}})();const _="https://sound-wave.b.goit.study/api";c.defaults.baseURL=_;async function z(t=10,e=1){try{return(await c.get("/feedbacks",{params:{limit:t,page:e}})).data}catch(s){throw console.error("Error fetching feedback list:",s.message,s.response?.status),s}}let g;const G=document.querySelector(".swiper-wrapper"),E=document.querySelector(".feedback-pagination");async function K(){try{(await z(10,1)).data.forEach(({rating:s,descr:n,name:a})=>{const o=X({rating:s,text:n,user:a});G.appendChild(o)}),Q()}catch(t){console.error("Oops...Error",t.message)}}function X({rating:t,text:e,user:s}){const n=document.createElement("div");n.classList.add("swiper-slide");const a=Math.round(t);return n.innerHTML=`
  ...
  `,n}function J(t){let s="";for(let n=1;n<=5;n++){const a=n<=t?"star-filled":"star-outline";s+=`
     <svg class="star-icon ${a}" width="24" height="24">
        <use href="icons/symbol-defs.svg#${n<=t?"icon-star-filled":"icon-star-outline"}"></use>
      </svg>
    `}return s}function Q(){g=new R(".feedback-swiper",{loop:!1,navigation:{nextEl:".feedback-button-next",prevEl:".feedback-button-prev"},grabCursor:!0,on:{slideChange:V}}),I(),Z()}function I(){E.innerHTML="";const t=g.slides.length,e=g.activeIndex;if(t===0)return;const s=0,n=Math.floor(t/2),a=t-1;[s,n,a].forEach(l=>{const i=document.createElement("span");i.classList.add("swiper-pagination-bullet"),i.setAttribute("data-slide-index",l),e===l&&i.classList.add("swiper-pagination-bullet-active"),E.appendChild(i)})}function V(){I()}function Z(){E.addEventListener("click",t=>{const e=t.target.closest(".swiper-pagination-bullet");if(!e)return;const s=parseInt(e.getAttribute("data-slide-index"),10);g.slideTo(s)})}K();c.defaults.baseURL="https://sound-wave.b.goit.study/api";const h=8;let T=1;window.addEventListener("DOMContentLoaded",tt);function u(t){return document.querySelector(t)}async function tt(){const t=u(".artists-list"),e=u(".artists-load-more-btn"),s=u(".loader");if(!t||!e){console.error("[artists] .artists-list або .artists-load-more-btn не знайдено у DOM");return}await et(1),e.addEventListener("click",n);async function n(){a(!0),e.style.display="none",T+=1;const o=await H(T,h);t.insertAdjacentHTML("beforeend",P(o)),q(e,o.length),a(!1),e.style.display!=="none"&&(e.style.display="flex")}function a(o){s?.classList.toggle("visually-hidden",!o)}}async function et(t){const e=u(".artists-list"),s=u(".artists-load-more-btn"),n=u(".loader");n?.classList.remove("visually-hidden");const a=await H(t,h);e.innerHTML=P(a),q(s,a.length),n?.classList.add("visually-hidden")}async function H(t=1,e=h){try{const{data:s}=await c.get("/artists",{params:{page:t,limit:e},headers:{Accept:"application/json"}});return Array.isArray(s)?s:Array.isArray(s?.artists)?s.artists:Array.isArray(s?.results)?s.results:s?.items??[]}catch(s){return console.error("[artists] fetchArtists error:",s),[]}}function q(t,e){t.style.display=e<h?"none":"flex"}function P(t=[]){return t.map(({_id:e,genres:s,strArtist:n,strArtistThumb:a,strBiographyEN:o})=>{const l=a||"./img/placeholders/artist@1x.jpg",i=n||"Unknown",d=st(s).map(y=>`<li class="artists-genres-item">${x(nt(y))}</li>`).join("");return`
<li class="artists-card-item">
  <img class="artists-image" src="${l}" alt="${x(i)}" loading="lazy" />
  <ul class="artists-genres-list">${d}</ul>
  <p class="artists-name">${x(i)}</p>
...
`;r.modalContent.insertAdjacentHTML("afterbegin",F);const C=document.querySelector("#bio-data > .artist-details-info"),S=document.querySelector("#bio-load-more > .bx");document.querySelector("#bio-load-more").addEventListener("click",()=>{v=!v,v?(C.textContent=f,S.classList.replace("bx-caret-down","bx-caret-up")):(C.textContent=M,S.classList.replace("bx-caret-up","bx-caret-down"))})}function dt(t){const{tracks:e,strAlbum:s}=t;let n="";return e&&e.length&&(n=`
          ...
          <ul>
          ${e.map(({strTrack:a,intDuration:o,movie:l})=>`
            <li class="song">
  ...
  `}async function ut(t="65ada69eaf9f6d155db48612"){r.modal.classList.add("active"),document.body.classList.add("modal-open"),r.modalContent.innerHTML='<div class="modal-loading">Loading artist details...</div>',O();try{const[e,s]=await Promise.all([ot(t),rt(t)]);r.modalContent.innerHTML="",ct(e,s)}catch(e){r.modalContent.innerHTML=`<div class="modal-error">Failed to load artist details due to ${e}.</div>`}finally{b(),r.modalContentWrapper.scrollHeight>r.modalContentWrapper.offsetHeight&&(r.modalContentWrapper.style.overflowY="scroll",r.modalContentWrapper.style.overflowX="hidden")}}function pt(t){let e="";if(t.target.nodeName==="BUTTON"&&t.target.classList.contains("artists-learn-more-card-btn"))e=t.target.getAttribute("data-artist-id")||"",ut(e),r.modalClose.addEventListener("click",m),r.modal.addEventListener("click",s=>{s.target===r.modal&&m()});else return}function mt(){r.modalClose.removeEventListener("click",m),r.modal.removeEventListener("click",t=>{t.target===r.modal&&m()})}function m(){r.modal.classList.remove("active"),document.body.classList.remove("modal-open"),mt(),setTimeout(()=>{r.modalContent.innerHTML=""},300)}document.addEventListener("DOMContentLoaded",()=>{document.querySelectorAll(".logo, .logo-mobile").forEach(i=>{i.addEventListener("click",d=>{d.preventDefault(),window.location.reload()})}),window.addEventListener("scroll",()=>{const i=document.querySelector(".header");window.scrollY>50?i.classList.add("scrolled"):i.classList.remove("scrolled")});const e=document.getElementById("open-menu"),s=document.getElementById("close-menu"),n=document.getElementById("mobile-menu"),a=n?.querySelectorAll(".nav-list-mobile a")||[];if(!e||!s||!n)return;const o=()=>{n.classList.add("open"),n.classList.remove("hidden"),document.body.style.overflow="hidden",e.setAttribute("aria-expanded","true")},l=()=>{n.classList.remove("open"),setTimeout(()=>n.classList.add("hidden"),300),document.body.style.overflow="",e.setAttribute("aria-expanded","false"),e.focus()};e.addEventListener("click",o),s.addEventListener("click",l),a.forEach(i=>i.addEventListener("click",l)),document.addEventListener("click",i=>{n.classList.contains("open")&&!n.contains(i.target)&&i.target!==e&&l()}),document.addEventListener("keydown",i=>{i.key==="Escape"&&n.classList.contains("open")&&l()}),r.artists&&r.artists.addEventListener("click",pt),window.addEventListener("hashchange",()=>{n.classList.contains("open")&&l()}),window.addEventListener("resize",()=>{window.innerWidth>=768&&n.classList.contains("open")&&l()})});document.addEventListener("keydown",t=>{t.key==="Escape"&&r.modal.classList.contains("active")&&m()});
